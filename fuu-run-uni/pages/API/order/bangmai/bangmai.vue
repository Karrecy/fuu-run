<template >
	...
	<scroll-view scroll-y style="height: 100vh;">
		<div class="box">
			<nut-tabs animated-time="0" background="linear-gradient(180deg, rgb(220 240 255) 0%, rgba(255,255,255,1) 100%);" v-model="tabValue">
			  <nut-tab-pane title="指定地点">
			  ...
			  <nut-tab-pane title="就近购买">
			    <nut-cell title="跑腿员将在周边合适地点购买" >
			    	<template #icon>
			    			<image class="nut-icon" src="/static/icons/start.png"></image>
			    	</template>
			    </nut-cell>
		...
		<div class="box">
			...
			<nut-cell @click="showPricePopup = true" is-link title="商品预估价格" :desc="submitForm.estimatedPrice">
		...
		<template>
			...
			<nut-popup v-model:visible="showPricePopup" position="bottom" safe-area-inset-bottom>
				<div class="nut-picker__bar">
					<div class="nut-picker__title">商品价格</div>
				</div>
				<div class="custom-content">
					<nut-input v-model="currEstimatedPrice" type="digit">
						<template #left>
							<nut-icon name="/static/icons/RMB.png"></nut-icon>
						</template>
			
						<template #right>
							<nut-button @click="onConfimEstimatedPrice" type="primary" size="small">确定填入</nut-button>
						</template>
